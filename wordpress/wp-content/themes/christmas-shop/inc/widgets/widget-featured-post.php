...
// register christmas_shop_Featured_Post widget
function christmas_shop_register_featured_post_widget() {
    register_widget( 'Christmas_Shop_Featured_Post' );
}
add_action( 'widgets_init', 'christmas_shop_register_featured_post_widget' );
 
 /**
 * Adds Christmas_Shop_Featured_Post widget.
 */
class Christmas_Shop_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'christmas_shop_featured_post', // Base ID
			__( 'PROSYS: Featured Post', 'christmas-shop' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'christmas-shop' ), ) // Args
		);
	}
            ...
            while( $qry->have_posts() ){
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'christmas-shop-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
		$postlist[0] = array(
    		'value' => 0,
    		'label' => __('--Choose--', 'christmas-shop'),
    	);
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'christmas-shop' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'christmas-shop' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'christmas-shop' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'christmas-shop' );
...
} // class Christmas_Shop_Featured_Post