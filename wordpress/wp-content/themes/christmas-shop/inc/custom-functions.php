...
if ( ! function_exists( 'christmas_shop_setup' ) ) :
	...
	function christmas_shop_setup() {
	    ...
	    add_image_size( 'christmas-shop-slider', 1140, 450, true );
	    ...
	    add_image_size( 'christmas-shop-welcome', 480 , 360, true );
...
if( ! function_exists( 'christmas_shop_admin_notice' ) ) :
/**
 * Addmin notice for getting started page
*/
function christmas_shop_admin_notice(){
    global $pagenow;
    $theme_args      = wp_get_theme();
    $meta            = get_option( 'christmas_shop_admin_notice' );
        ...
        <div class="welcome-message notice notice-info">
            <div class="notice-wrapper">
                <div class="notice-text">
                    <h3><?php esc_html_e( 'Congratulations!', 'christmas-shop' ); ?></h3>
                    <p><?php printf( __( '%1$s is now installed and ready to use. Click below to see theme documentation, plugins to install and other details to get started.', 'christmas-shop' ), esc_html( $name ) ) ; ?></p>
                    <p><a href="<?php echo esc_url( admin_url( 'themes.php?page=christmas-shop-getting-started' ) ); ?>" class="button button-primary"><?php esc_html_e( 'Go to the getting started.', 'christmas-shop' ); ?></a></p>
                    <p class="dismiss-link"><strong><a href="?christmas_shop_admin_notice=1"><?php esc_html_e( 'Dismiss', 'christmas-shop' ); ?></a></strong></p>
                </div>
            </div>
        </div>
    <?php }
...
add_action( 'admin_notices', 'christmas_shop_admin_notice' );



if( ! function_exists( 'christmas_shop_update_admin_notice' ) ) :
/**
 * Updating admin notice on dismiss
*/
function christmas_shop_update_admin_notice(){
    if ( isset( $_GET['christmas_shop_admin_notice'] ) && $_GET['christmas_shop_admin_notice'] == '1' ) {
        update_option( 'christmas_shop_admin_notice', true );
    }
}
endif;

add_action( 'admin_init', 'christmas_shop_update_admin_notice' );


if( ! function_exists( 'christmas_shop_admin_scripts' ) ) :
/**
 * Enqueue admin scripts and styles.
 */
function christmas_shop_admin_scripts(){
    
    wp_enqueue_style( 'christmas-shop-admin-style', get_template_directory_uri() . '/inc/css/admin.css', array(), CHRISTMAS_SHOP_THEME_VERSION );
    
}
endif;

add_action( 'admin_enqueue_scripts', 'christmas_shop_admin_scripts' );